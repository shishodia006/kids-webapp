      ...
      <section className="mx-auto my-auto w-full max-w-[430px] rounded-[22px] border border-zinc-200 bg-white px-6 py-7 text-left shadow-[0_20px_70px_rgba(38,26,7,0.14)]">
        ...
        {step === "reset" && (
          <form onSubmit={resetPassword} className="mt-5 grid gap-3">
            ...
            <input required value={password} onChange={(event) => setPassword(event.target.value)} className="h-[48px] rounded-[14px] border-[2px] border-[#cbd6e6] bg-[#e8f0fd] px-4 text-sm font-semibold outline-none" placeholder="New password" type="password" />
            <input required value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} className="h-[48px] rounded-[14px] border-[2px] border-[#cbd6e6] bg-[#e8f0fd] px-4 text-sm font-semibold outline-none" placeholder="Confirm new password" type="password" />
        ...
        {status && <p className="mt-4 text-sm font-black text-[#6655cf]">{status}</p>}
