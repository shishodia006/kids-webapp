...
import { createAdminSessionToken, getSessionCookieOptions, SESSION_COOKIE_NAME } from "@/lib/auth/session";
...
export async function POST(request: Request) {
  try {
    ...
    response.cookies.set(SESSION_COOKIE_NAME, createAdminSessionToken(ADMIN_EMAIL), getSessionCookieOptions());
    return response;
  } catch (error) {
    console.error(error);
